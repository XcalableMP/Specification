...
\gitem{\XMPF}
 ...
 The {\XMP} specification for a base language {\Fort}.

\gitem{\XMPC}

 The {\XMP} specification for a base language {\C}.
...
\gitem{global construct}
 ...
 loops, array assignments or tasks.
...
\gitem{local}
 ...
 instance of it on each node in the executing node set.
...
%
 Note that the executing node set of the main routine is the entire node
 set.
...
\gitem{node number}
 ...
 A unique number assigned to each node, which starts from one and
 corresponds to its position within the linearly-ordered entire node
 set.
...
\gitem{image index}
 ...
 node in the node array with which the coarray is associated. The
