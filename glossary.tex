...
An instance of an XcalableMP program corresponding to a node respectively.
...
whose range is one to the size of the image set.
...
is the entire image set.

\gitem{allocation image set}

An image set that the coarray data object is allocated on.

The allocation image set for a non-allocatable [F] or a static [C] 
coarray variable is the entire image set.
Otherwise, 
the allocation image set for an allocatable [F] or an auto [C] 
coarray variable is the executing image set where it is allocated
