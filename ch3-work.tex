...
\item[Example 1]
	   ...
	   nodes {\tt nd(1)} through {\tt nd(8)}. A task defined by
	   the {\tt task} construct is executed only on {\tt nd(1)} and
	   defines the copies of {\tt a} and {\tt b} on a node {\tt
...
\item[Example 2]
	   ...
	   executed by the owner of the array element {\tt a(:,j)} or
	   {\tt a[j][:]}.
...
\verb![F]! & \verb|!$xmp| {\tt loop} {\openb} \verb|(| {\it loop-index}
 {\openb}, {\it loop-index}{\closeb}... \verb|)| {\closeb} {\bsquare} \\
 & \hspace{5cm}{\bsquare} {\tt on} \{{\it nodes-ref} $\vert$ {\it
     template-ref}\} {\openb} {\it reduction-clause} {\closeb}... \\
 & {\it do-loops} \\
 & \\
...
it is specified in the sequence. The sequence can be considered to
...
associated loop is distributed so that the iteration (set) indexed by
the the sequence of {\it loop-index}'s is executed by a node
...
to the last one at the end of it.
...
\mytextcolor{red}{replacing each {\it loop-index} in the {\it
...
\Term{location-variable}}'s are set after executing the {\tt loop}
...
a node determined by the {\tt on} clause.
