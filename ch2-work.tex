...
the immediately surrounding {\tt tasks} construct, where the evaluation
...
\item The node set specified by {\it nodes-ref} or {\it template-ref}
      in the {\tt on} clause must be a subset of the parent executing
      node set.
\end{itemize}
...
Conceptually, {\it nodes-ref} or {\it template-ref} of each task
immediately surrounded by a {\tt tasks} construct is evaluated at the
entry of the {\tt tasks} construct.
...
\underline{Explanation to be added.}
...
\begin{itemize}
\item The template section specified by {\it template-ref} must have the
