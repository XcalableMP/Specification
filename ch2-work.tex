...
\verb![F]! & \verb|!$xmp| {\tt loop} {\openb} \verb|(| {\it loop-index}
 ...
 on-ref} {\openb} {\it reduction-clause} {\closeb} \\
\verb![C]! & \verb|#pragma xmp| {\tt loop} {\openb} \verb|(| {\it
     loop-index} {\openb}, {\it loop-index}{\closeb}... \verb|)|
     {\closeb} {\tt on} {\it on-ref} {\openb} {\it reduction-clause}
     {\closeb} \\
\end{tabular}
...
the execution of their iterations among nodes determined by the {\tt on}
clause.
% inserted by Sakagami,H. 09/11/13
...
When {\it on-ref} is a {\it template-ref}, according to the distribution
...
this node set as a (new) executing node set. Therefore, before the {\tt
...
When {\it on-ref} is a {\it nodes-ref}, the node set associated with the
loop index is created as the new executing node set to execute the iteration
of the loop index.
...
type specified by {\it reduction-kind} for the specified local reduction
variable is executed just after the execution of the loop. 
...
% modified by Sakagami,H. 09/11/13
\item If {\it reduction-kind} is either {\tt FIRSTMAX}, {\tt FIRSTMIN},
      {\tt LASTMAX}, or {\tt LASTMIN}, then {\it reduction-spec} must
      have one or more {\it location-variable}.
% inserted by Sakagami,H. 09/11/13
...
shown below, but will be faster because the iterations executed in each
node are decided before executing the loop.
...
Since the first dimension of template {\tt t} is not distributed, this
loop is distributed only for the second dimension. This example is
syntactically equivalent to the {\tt task} construct shown below.
...
% modified by Sakagami,H. 09/11/13
The distribution for the multi-dimensional array in the nested loop can
be described 
using the sequence of {\it loop-index} in one {\tt \Directive{loop}}
construct. This example is equivalent to the loop shown
below, but will run faster because the iterations 
to be executed by each node are decided outside of the nested loop. Note
that the inner template index set is included in the outer template
index set. 
...
where each sub-assignment to an element is executed by the node
determined by the {\tt on} clasue.
