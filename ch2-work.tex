...
When {\it on-ref} is a {\it template-ref}, the associated loop is
distributed so that the iteration (set) indexed by the the sequence of {\it 
loop-index}'s  is executed by the node onto which the template element
specified by the {\it template-ref} is distributed.
...
When {\it on-ref} is a {\it nodes-ref}, the associated loop is
distributed so that the iteration (set) indexed by the the sequence of {\it 
loop-index}'s  is executed by the node specified by the {\it nodes-ref}.
...
{\it reduction-variabl} is executed just after the execution of the loop
nest.
...
with the same {\it reduction-ref} and an {\tt on} clause obtained from
{\it on-ref} by replacing:
%
\begin{itemize}
 \item ``{\tt :}'' with ``{\tt *}'', and
 \item {\it loop-index} with a triplet representing the range of its
       value.
\end{itemize}
%
...
MAX} or {\tt MIN} reduction operation, the {\tt location-variable}'s are
...
\begin{itemize}
 \item For {\tt FIRSTMAX} and {\tt FIRSTMIN}, they are set to their
       ...
       the {\tt reduction-variable} takes the value of the reduction
...
%       a node must not be included in more than one node set. 
 ...
 \item The template specified by {\tt template-ref} must be fixed 
...
\end{Fexample}
	   ...
	   distributed, only the {\tt j} loop, which is aligne with the
