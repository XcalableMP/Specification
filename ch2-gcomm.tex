...
an on clause. If an on clause is not specified, then the current executing
node set is used.
%
...
reduction variable becomes the same. 

% inserted by Sakagami,H. 09/11/13 ----- start ----                            
When {\it template-ref} is specified in the {\tt on} clause, the operation
is performed in a node set that consists of nodes associated with the specified template.
...
When {\it template-spec} is ``:'', nodes for all template elements in the corresponding dimension perform the reduction operation.

When {\it node-ref} is specified in the {\tt on} clause, the operation
is performed in the specified node set. Therefore, before the {\tt \Directive{reduction}} construct is executed, the referenced node set must be fixed.

When the {\tt on} clause is omitted, the operation is performed in the current executing node set.
...
partial sum of the variable,                                                              
and the reduction operation calculates the total sum of the variable. The      
total sum is stored in the variable in each node.                              
                                                                               
% modified by Sakagami,H. 09/11/13                                             
The second example computes the maximum value of the variable {\tt aa} in the node set that consists of nodes associated with the range of the second dimension of template {\tt t}.                  

% modified by Sakagami,H. 09/11/13                
In the third example, the minimum value of the variable {\tt bb} in the node 
set is specified by {\tt p(10:30)}. This example is equivalent to the       
...
However, this code can be modified with the {\tt \Directive{reduction}} construct  as follows:
...
This code executes a reduction operation on the array {\tt w} only once, which may result in faster operation. 
...
variable for broadcast is referred to as the ÅeÅe\Term{broadcast variable}". After
executing this directive, the value of the broadcast variable becomes the 
same as the value of the source node. If the {\tt from} clause is omitted,
then the first node of the node set is a source node. If the {\tt on} clause is
omitted, then the operation is performed in the current executing node set.
