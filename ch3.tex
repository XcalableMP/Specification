...
is omitted, it is assumed to be the rest size of the dimension of the
array. When {\it step} is omitted, it is assumed to be 1.
}
...
\mytextcolor{red}{
The value of each element of the result of the rhs expression is
assigned to the corresponding element of the lhs array 
section.
%
When an operator or an elemental function (see section
\ref{094142_25Sep13}) is applied to array sections in the rhs
expression, it is evaluated to an array section that has the same shape
as of the operands or arguments and each element of which is the result of
the operator or function applied to corresponding element of the
operands or arguments, where a scalar object is assumed to be an array
section that has the same shape as of the array section(s) and each
element of which has its value.}
