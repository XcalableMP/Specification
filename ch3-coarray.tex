...
programming. Particularly in the case of {\Fort} as the base language,
the notations are compatible with that of Fortran 2008.

\section{Coarray in XcalableMP}
...
\Syntax{coarray}
\begin{tabular}{ll}
\verb![F]! & {\it type-declaration-stmt} {\openb}{\tt on} {\it nodes-ref}{\closeb}\\
...
of a variable.
%
Terms realated to coarray in {\XMP} (e.g. coshape, coindex,
...
Note that an acutual coarray and its associating dummy coarray must be
...
\begin{itemize}
 \item \verb![F]! All of the variables declared by {\it
       type-declaration-stmt} with the {\tt on} clause must have the
...
\end{Fexample}
	   ...
	   formal argument {\tt aa} and its declaration requires an {\tt
...
%$
	   ...
	   The entire node set contains 200 nodes, on which coarray {\tt
	   ...
	   nothing to the image of coarrays.
...
A sequence of {\it int-expr} in a square bracket in the Fortran base
language and a sequence of {\tt [}{\it
int-expr}{\tt ]} preceded by a colon in the C base language determine
the image index for a coarray to be accessed.

Note that a coarray cannot be directly accessed by a node outside the
node set on which it is declared, unlike that in Fortran 2008.
...
node set) and copies it into {\tt A}.
