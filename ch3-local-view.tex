...
This construct prohibits statements following it from being executed
...
It is assumed that the left code is executed by the node {\tt p(1)} and
the right by {\tt p(2)}.

The {\tt post} construct in the left matches
the {\tt wait} construct in the right because each {\it nodes-ref}
represent each other and both {\it tags} have the same value of 1.
%
...
It is assumed that this code is executed by the node {\tt p(3)}.

The {\tt post} construct in the left code of Example 1 can match
this {\tt wait} construct because both {\it nodes-ref} and
	   {\it tag} are omitted in it.
...
If {\it tag} is omitted, the {\tt \Directive{wait}} construct can match at
...
%
If both {\it tag} and {\it nodes-ref} are omitted, the {\tt
...
associated structured block to a single node at a time.
...
This construct specifies a {\it critical region}, at the beginnng of
