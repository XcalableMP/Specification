...
{\it local-array-name} is a ``local alias'' to the global array
...
An array specified by {\it local-array-name} may be a coarray.
...
\Directive{post} construct start to be executed.
...
arguments of the waiting node and the same value as {\it tag} the
...
\section{Array Assignment Statement in C}
...
\section{Descriptor of Global Data in C}
\label{sec:Descriptor of Global Data in C}

\subsection{Operator which indicate descriptor}

When distribution of Global data is defined, query function which have 
operator {\tt xmp\_desc\_of} as argument can return some descriptor information. 
...
In the C base language, it is possible to allocate global arrays at
runtime only when they are one-dimensional.
%
...
details of the {\tt xmp\_desc\_of} library procedure, refer to
Chapter \ref{chap:Intrinsic and library procedures}.
