...
local data object as an alias to the local section of a distributed
array.
...
array that is not either an allocatable array or an array pointer can be
declared if it is specified as a ``local alias'' by the {\tt
local\_alias} directive.

In {\XMPC}, the address-of operator applied to global data
...
nodes-ref} and {\it tag} on a node (called a {\it \Term{waiting node}})
...
assumed that {\it base} is one, {\it length} is the size of the
...
implementation-dependent, and defined in the {\tt xmp.h} heder file in
{\XMPC}.
