...
\section{Pointer to Global Data in C}

\subsection{Name of Global Array}

The name of a global array is considered to represent an abstract object
in the {\XMP} language. It is not interpreted as the pointer to it,
...
\section{Dynamic Allocation of Global Data in C}

In C, it is possible to allocate global arrays at runtime only when they
are one-dimensional.
%
...
\begin{enumerate}
 ...
 \item Allocate a storage of the global size with the {\tt xmp\_malloc}
       runtime library routine and assign the result value to the
       pointer on each node.
\end{enumerate}
%
The specification of {\tt xmp\_malloc} is described in Chapter
\ref{chap:Intrinsic and library procedures}.

\subsection*{Example}

A pointer {\tt p} to a float is declared in line 5 and aligned with a
template {\tt t} in line 6. {\tt t} is initially undefined and fixed by
the {\tt template\_fix} directive in line 10. The storage for the global
data, that is, each of the local section is allocated with {\tt
xmp\_malloc} and {\tt p} is set to point it on each node in line 12. For
details of the {\tt xmp\_desc\_of} runtime library routine, refer to
...
P = (float *)xmp_malloc(xmp_desc_of(p), N);
\end{Cexample}
