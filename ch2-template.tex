...
\begin{tabular}{ll}
\verb![F]! & \verb|!$xmp| {\it template-name} \verb|(| {\it template-spec} 
{\openb}, {\it template-spec} {\closeb}... \verb|)| \\
& \\
\verb![C]! & \verb|#pragma xmp|  {\it template-name} \verb|(| {\it template-spec} 
{\openb}, {\it template-spec} {\closeb}... \verb|)| \\
\end{tabular}
\vspace{0.3cm}
...
The \Term{template reference} expression specified in the {\tt on}
...
reference refers to a subset of a node set onto which the subset of the
...
that is distributed onto it. ``{\tt *}'' in the template reference is
...
\begin{tabular}{ll}
 \hspace{0.5cm} & {\tt scalar-int-variable} {\openb} \{ {\tt +} $\vert$
