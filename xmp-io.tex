...
%    \caption{aaa}
   ...
   input and output, must be executed on the same execution node set. 
   ...
   \begin{table}[h]
	...
	!$xmp align a(*,i) with t(i)
      ...
      & local variable &  The value read from the file is replicated into the
	      local array on all execution nodes. The file positioning seeks by
      ...
      output item & name of global array & The value referred from the global data 
	      ...
	      The file positioning seeks by the size of the data.\footnote{xxx}\\
      \cline{2-3}
      & local variable, expression & The value evaluated on a node of the execution nodes is written to the file.
...
%    \caption{aaa}
   ...
   Atomic I/O statements read/write shared files exclusively among execution nodes
   ...
   atomic CLOSE statement executed on the same execution node set.
   Atomic READ and WRITE statements can be executed on any single node of the
   same execution node set.
   ...
   direct CLOSE statement executed on the same execution node set.
   ...
   \begin{description}
    ...
    \item[xmp\_array\_t] Mapping descriptor for a global array variable,
	       ...
	       Mapping descriptor of global array A is gotten with
   ...
   \paragraph{xmp\_fopen\_all}   
   opens a global I/O file. Collective (global) execution.
...
%    \caption{aaa}
   ...
   File view is initialized. The value of shared and individual file pointers 
   ...
   \clearpage
   \paragraph{xmp\_fclose\_all} 
   closes a global I/O file. Collective (global) execution.
...
%    \caption{aaa}
   ...
   \paragraph{xmp\_fseek}
   sets the individual file pointer in the file structure. Local execution.
...
%    \caption{aaa}
   ...
   \paragraph{xmp\_fseek\_shared}
   sets the shared file pointer in the file structure. Local execution.
...
%    \caption{aaa}
   ...
   \paragraph{xmp\_ftell}
   inquires the position of the individual file pointer in the file structure. Local execution.
...
%    \caption{aaa}
   ...
   \paragraph{xmp\_ftell\_shared}
   inquires the position of shared file pointer in the file structure. Local execution.
...
%    \caption{aaa}
   ...
   \paragraph{xmp\_file\_sync\_all}
   guarantees completion of access to the file from nodes sharing the file.
...
%    \caption{aaa}
   ...
   \paragraph{ xmp\_file\_set\_view }
   sets a file view to the file. Collective (global) execution.
   ...
   \begin{table}[h]
    \begin{center}
     \begin{tabular}{|l|r|p{70mm}|}
      ...
      & xmp\_array\_t ap & array descriptor \\ \cline{2-3}
...
%    \caption{aaa}
  ...
  The file view of distributed array $ap$ limited to range $rp$ is set into file structure $fh$.
   
   \paragraph{ xmp\_file\_clear\_view }
   clears the file view. Collective (global) execution.
...
%    \caption{aaa}
   ...
   \paragraph{ xmp\_fread\_all }
   reads the same data from the position of the shared file
   pointer onto the all execution nodes. Collective (global) execution.
...
%    \caption{aaa}
   ...
   \paragraph{ xmp\_fwrite\_all }
   writes individual data on the all execution nodes to the position of the shared file
...
%    \caption{aaa}
   ...
   \paragraph{ xmp\_fread\_darray\_all }
   reads data cooperatively to the global array from the
...
%    \caption{aaa}
   ...
   Data is read from the file to distributed array $ap$ limited to range $rp$.

   \paragraph{ xmp\_fwrite\_darray\_all }
   writes data cooperatively from the global array to the
   position of the shared file pointer.
...
%    \caption{aaa}
   ...
   Data is written from distributed array $ap$ limited to range $rp$ to the file.
   ...
   \paragraph{ xmp\_fread\_shared }
   exclusively reads local data form the position of the shared file
...
%    \caption{aaa}
   ...
   \paragraph{ xmp\_fread\_shared }
   exclusively writes local data to the
...
%    \caption{aaa}
   ...
   \paragraph{ xmp\_fread }
   reads data from the position of the individual file
...
%    \caption{aaa}
   ...
   \paragraph{ xmp\_fread }
   writes data to the position of the individual file
