...
\section{Extended Mapping Inquiry Procedures}

In this section, the extended mapping inquiry procedures, which are
implementation-dependent, are shown.
Specifications of the procedures below are from the Omni XcalableMP
...
The {\tt xmp\_array\_gtol} routine translates an index (specified by
...
\begin{itemize}
 \item {\tt d} is a descriptor, that is, an object of type {\tt
       type(xmp\_desc)}, in {\XMPF}, or {\tt xmp\_desc\_t},
...
The {\tt xmp\_array\_lsize} routine provides the local size of each
...
The {\tt xmp\_array\_laddr} routine provides the local address of the
target global array. 
...
The {\tt xmp\_array\_lead\_dim} routine provides the leading dimension
